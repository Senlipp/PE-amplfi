...
def main():
    ...
    flow_obj.embedding_net.eval()
    flow_obj.embedding_net.qconfig = quantization.get_default_qconfig('x86')
    flow_obj.embedding_net.fuse_model()
    embedding_prepared = quantization.prepare(flow_obj.embedding_net)
    embedding_prepared.to('cpu')
    data_loader = sig_dat.val_dataloader()
    ...
    with torch.no_grad():
        ...
        for batch in data_loader:
            ...
            embedding_prepared(inputs)  
            
            if i == 3:
                break
    
    embedding_quantized = quantization.convert(embedding_prepared)
    flow_obj.embedding_net=embedding_quantized
    
    print(flow_obj.embedding_net)
    torch.save(flow_obj.state_dict(), 'quantized_pqt_model_state.pth')
