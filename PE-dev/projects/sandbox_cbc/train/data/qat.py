...
def main():
    ...
    flow_obj.embedding_net.qconfig = torch.ao.quantization.get_default_qat_qconfig('x86')
    
    flow_obj.embedding_net.fuse_model()
    flow_obj.embedding_net = torch.ao.quantization.prepare_qat(flow_obj.embedding_net.train())

    trainer.fit(model=flow_obj, datamodule=sig_dat)
    
    flow_obj.embedding_net.eval()
    flow_obj.embedding_net = torch.ao.quantization.convert(flow_obj.embedding_net)

    torch.save(flow_obj.state_dict(), 'quantized_qat_model_state.pth')
